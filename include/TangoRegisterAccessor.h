...
namespace ChimeraTK {
  ...
  template<typename UserType, typename TangoType>
  TangoBackendRegisterAccessor<UserType, TangoType>::TangoBackendRegisterAccessor(
      boost::shared_ptr<TangoBackend> backend_, TangoRegisterInfo info, const std::string& registerPathName,
      size_t numberOfWords, size_t wordOffsetInRegister, AccessModeFlags flags)
  : NDRegisterAccessor<UserType>(registerPathName, flags), backend(std::move(backend_)), registerInfo(std::move(info)) {
  ...
  template<typename UserType, typename TangoType>
  void TangoBackendRegisterAccessor<UserType, TangoType>::doPreRead(TransferType) {
    if(!backend->isOpen()) {
      throw ChimeraTK::logic_error("Read operation not allowed while device is closed.");
    }
    if(!isReadable()) {
      throw ChimeraTK::logic_error("Try to read from write-only register \"" + registerInfo.getRegisterName() + "\".");
    }
  }
